<html>
<body>
<div class="container" style="text-align: center">
<h2>Hello Users!</h2>

<h2>Add</h2>
<form action="add">
First Number :<input type="number" name="t1" style=" border: 2px solid black"><br>
Second Number :<input type="number" name="t2" style=" border: 2px solid black"><br>

<button type="submit" style="color:red;font-size: 16px;border:2px solid black;width: 63px;margin-top: 13px">Add</button>

</form>
<h2>Subtract</h2>
<form action="subtract">
First Number :<input type="number" name="t3" style=" border: 2px solid black" ><br>
Second Number :<input type="number" name="t4" style=" border: 2px solid black"><br>

<button type="submit" style="color:red;font-size: 16px;border:2px solid black;width: 75px;margin-top: 13px">Subtract</button>
</form>
</div>

</body>
</html>
